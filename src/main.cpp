...
int main() {
	...
	while (window.isOpen())
	{
		...
		int velY = 0, velX = 0;
		if (goingUp && circleBody->GetPosition().y > 0) {
			velY = -1000;
		}else if (goingDown && circleBody->GetPosition().y < window.getSize().y - 2*circleBody->GetFixtureList()->GetShape()->m_radius) {
			velY = 1000;
		}

		if (goingLeft && circleBody->GetPosition().x > 0) {
			velX = -1000;
		}else if (goingRight && circleBody->GetPosition().x < window.getSize().x - 2 * circleBody->GetFixtureList()->GetShape()->m_radius) {
			velX = 1000;
		}
		...
		circleBody->SetLinearVelocity(b2Vec2(velX, velY));
